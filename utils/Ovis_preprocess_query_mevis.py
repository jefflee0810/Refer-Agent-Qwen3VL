...
def main(args):
    ...
    # `random.shuffle` above is deterministic (seed=42), so the global ordering is
    # the same across invocations; striding by num_shards gives disjoint subsets.
    ...
    for pid in range(thread_num):
        ...
        if key in result_dict:
            ...
            ckpt = os.path.join(output_dir, f'mevis_result_shard{shard_id}_pid{pid}.json')
    ...
    if num_shards == 1:
        out_name = "mevis_query_qa.json"
    else:
        out_name = f"mevis_query_qa_shard{shard_id}.json"
...
def sub_processor(lock, pid, args, data, save_path_prefix, img_folder, video_list, result_dict, shard_id=0):
    ...
    checkpoint_path = os.path.join(save_path_prefix, f'mevis_result_shard{shard_id}_pid{pid}.json')
    results = {}
